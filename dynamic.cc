...
//	mac.SetBlockAckInactivityTimeoutForAc(AC_BE, 3);
	...
	apDevices = wifi.Install(phy, mac, wifiApNodes);
        ...
        Ptr<WifiPhy> clPhy;
        int i,j;
         apPhy = ((WifiNetDevice*)PeekPointer(apDevices.Get(0)))->GetPhy();
	        apPhy->SetChannelNumber(6);
        for(i=1;i<numberOfAps;i++){
               if(i%3==1)
                j=1;
               if(i%3==2)
                j=6;
               if(i%3==0)
                j=11;       
               apPhy = ((WifiNetDevice*)PeekPointer(apDevices.Get(i)))->GetPhy();
               apPhy->SetChannelNumber(j);
        }        
...
//	mac.SetBlockAckThresholdForAc(AC_BE, 0);
	...
	staDevices = wifi.Install(phy, mac, ueNodes);
        ...
        //..................../* Determine Closest AP for each Client*/......................................//
        int NumOfAP=numberOfAps;
        int ClientsAssocAP[numUes]; 
        int NumClientsPerAP[NumOfAP];
        float APpositions[NumOfAP][2]; 

        Ptr<MobilityModel> ClientsLoc;
        Ptr<MobilityModel> APLoc;
        Vector ClientPos;
        Vector APPos;
        ...
        int AP_Posx;
        int AP_Posy;
        // The following gets the position of the AP's
        for (int j=0;j<NumOfAP;j++){
                 APLoc = wifiApNodes.Get(j)->GetObject<MobilityModel>();
                 APPos = APLoc->GetPosition ();
                 AP_Posx=APPos.x;
                 AP_Posy=APPos.y;
                 APpositions[j][0]=AP_Posx;
                 APpositions[j][1]=AP_Posy;
        }
       // The following gets the position of the Clients and determeins which AP is closest
        int NumClients = numUes;
        float ClientAPDists[NumOfAP];
        int APClientTable[NumOfAP][NumClients];//[NumOfAP][NumClients]
	uint16_t tempChan=0;
	int ClientAP[NumOfAP];
        for (int i=0;i<NumClients;i++){
                ClientsLoc = ueNodes.Get(i)->GetObject<MobilityModel>();
                ClientPos = ClientsLoc->GetPosition ();
                Client_Posx=ClientPos.x;
                Client_Posy=ClientPos.y;
                for (int p=0;p<NumOfAP;p++){
                        ClientAPDists[p]=0;
                        ClientAP[p] = p;
                }

	        for (uint32_t j=0;j<NumOfAP;j++){
			        ClientAPDists[j]=((APpositions[j][0]-Client_Posx)*(APpositions[j][0]-Client_Posx))+((APpositions[j][1]-Client_Posy)*(APpositions[j][1]-Client_Posy));
                }

		 bubblesortAscend(ClientAPDists,ClientAP,NumOfAP);
  	  	 APClientTable[ClientAP[0]][i]=1;
  	  	 ClientsAssocAP[i]=ClientAP[0];
  	  	
  	  	 apPhy = ((WifiNetDevice*)PeekPointer(apDevices.Get(ClientAP[0])))->GetPhy();
  	  	 tempChan = apPhy->GetChannelNumber();            


  	  	 clPhy = ((WifiNetDevice*)PeekPointer(staDevices.Get(i)))->GetPhy();
  	  	 clPhy->SetChannelNumber(tempChan);
 	 ...
 	 for (int i=0;i<NumClients;i++){
 		...
 		//std::cout << "Client Number = " << i << " is associated with AP = " << ClientsAssocAP[i]<< std::endl;
 	 }
		...
		Vector pos = node->GetObject<MobilityModel>()->GetPosition();
   	        ...
   	        xvalue.numPoAues = 0; 
                ...
                for (uint32_t i = 0; i < enbNodes.GetN(); i++) {
                        Ptr<Node> nodeb = enbNodes.Get(i);
                        Vector posb = nodeb->GetObject<MobilityModel>()->GetPosition();
                        if (temp > (posb.x-pos.x)*(posb.x-pos.x)+(posb.y-pos.y)*(posb.y-pos.y)){
                               temp = (posb.x-pos.x)*(posb.x-pos.x)+(posb.y-pos.y)*(posb.y-pos.y);
                               x=posb.x;
                               y=posb.y;
                         }
                }
